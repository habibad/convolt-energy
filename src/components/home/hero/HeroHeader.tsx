...
export const HeroHeader: React.FC<HeroHeaderProps> = ({ introReady }) => {
  ...
  return (
    <header
      ref={headerRef}
      className="fixed top-0 left-0 right-0 z-40 h-[80px] md:h-[88px] w-full transition-colors duration-300 pointer-events-auto"
    >
      <div className="max-w-[1540px] mx-auto h-full px-[clamp(20px,4vw,64px)] flex items-center justify-between">
        ...
        <div ref={logoRef} className="opacity-0 flex items-center">
          ...
          >
            <div className="flex items-center tracking-tight text-[19px] sm:text-[21px] font-semibold text-[#101A1D]">
            ...
            <span className="text-[10px] sm:text-[11px] font-medium tracking-[0.28em] text-[#101A1D]/75 -mt-[3px] uppercase">
              ENERGY
            </span>
          </a>
        </div>
          ...
          <ul ref={navItemsRef} className="flex items-center space-x-7 xl:space-x-9">
            {navLinks.map((link) => (
              <li key={link.label} className="opacity-0">
                <a
                  href={link.href}
                  className="relative group py-2 text-[13px] xl:text-[14px] font-medium text-[#101A1D]/75 hover:text-[#101A1D] transition-colors duration-350"
                >
        ...
        <div ref={ctaRef} className="hidden sm:flex items-center opacity-0">
          <a
            href="#contact"
            className="group relative inline-flex items-center justify-center px-6 py-2.5 rounded-full bg-[#101A1D] text-[#F4F3EF] text-[13px] font-medium tracking-wide shadow-sm hover:bg-[#152327] transition-all duration-350"
          >
        ...
        <div className="flex sm:hidden items-center">
          <button
            ...
            className="p-2 text-[#101A1D] focus:outline-none"
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="sm:hidden absolute top-[80px] left-0 right-0 bg-[#F4F3EF]/95 backdrop-blur-md border-b border-black/10 px-6 py-8 shadow-xl flex flex-col space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              onClick={() => setMobileMenuOpen(false)}
              className="text-base font-medium text-[#101A1D] py-1 hover:text-[#63A75B] transition-colors"
            >
              {link.label}
            </a>
          ))}
          <div className="pt-4 border-t border-black/10">
            <a
              href="#contact"
              onClick={() => setMobileMenuOpen(false)}
              className="inline-flex w-full items-center justify-center py-3 rounded-full bg-[#101A1D] text-[#F4F3EF] text-sm font-medium"
            >
